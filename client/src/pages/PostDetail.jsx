...
const PostDetail = () => {
  ...
  // Find post or mock one if not found
        ...
        <div className="p-6 md:p-8 flex flex-col gap-8 h-full">
          ...
          <div className="flex items-center justify-between">
            <Link to={`/profile/${post.creator.username}`} className="flex items-center gap-3 group">
              <UserAvatar src={post.creator.avatar} alt={post.creator.name} />
              <div>
                <p className="font-bold text-white group-hover:text-violet-400 transition-colors">{post.creator.name}</p>
                <p className="text-xs text-gray-400">{post.followers ? post.followers : '12.4k'} followers</p>
              </div>
            </Link>
